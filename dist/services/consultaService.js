...
export async function createConsulta(pacienteId, dataHora) {
...
export async function updateConsultaStatus(consultaId, status) {
    const { data: consulta, error } = await supabase
        .from('consultas')
        .update({ status })
        .eq('id', consultaId)
...
export async function getProximaConsultaEnviadaPorPaciente(pacienteId) {
    const now = new Date();
    const { data, error } = await supabase
        .from('consultas')
        .select('*')
...
export async function verificarConsultaDuplicada(pacienteId, dataHora) {
    const { data, error } = await supabase
        .from('consultas')
        .select('id')
        .eq('paciente_id', pacienteId)
